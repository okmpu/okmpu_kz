...
# ----------------------------------------------------------------------------------------------------------------------
# content detail page
def content_detail_view(request, category_slug, sub_category_slug, section_slug, content_slug):
    ...
    content = get_object_or_404(Content.objects.select_related('category'), category=section, slug=content_slug)

    sub_categories = Category.objects.filter(parent=category)
    contents = (
        Content.objects
        .filter(category__parent__parent=category)
